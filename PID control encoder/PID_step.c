...
int main(void){
	FILE *fp = fopen("/home/pi/MJ_meca/PID_data/1-1/Enc/1X.txt","w");
	int loop = 1;
	wiringPiSetupGpio();
	...
	while(1){
		...
		if(checkTime - checkTimeBefore > LoopTime){
			...
			InterrPos = InterrPos + ((errPos+beferrPos)/2)*LoopTime; // divide by 1000?
			D_Loss = (errPos - beferrPos)/LoopTime; //divide by 1000?
			beferrPos = errPos;
